...
- 已增强（P1 规则）：结构化补救分句阶段会过滤短噪声片段与临时错误状态（如 timeout/5xx/报错），降低噪声写入 daily sections 的概率。
...
- 已调整：`guard-metrics-summary` 的 `Priority Focus` 默认展示 top 2，降低信息噪声、强化执行聚焦。
...
- 已增强：`guard-metrics-summary` 增加 `reason -> sessions` 覆盖统计（`sessions_with_hits_by_reason`），便于识别单会话异常与跨会话普遍问题。
...
- 已增强：dashboard 对 `unstructured_candidate` / `date_line_overflow` / `url_line_overflow` / `duplicate_line_overflow` 高发场景给出专项建议（结构化输出、时间线/链接信息回流 daily/history、去重重复 bullet）。
...
- 已增强：dashboard 高发阈值建议覆盖 `duplicate_bullets`，当重复 bullet 清洗量偏高时给出去重收敛提示。
- 已增强：当 `duplicate_bullets` 清洗量“非常高”时，dashboard 追加更强建议（收紧 consolidator，减少重复 bullet 生成）。
...
- 已实施 `memory_update` 异常变更阈值保护（guard）：
  - 候选更新超长时拒写（`candidate_too_long`，防止一次异常输出覆盖长期记忆）；
  - 候选更新包含 code block 时拒写（`contains_code_block`，防止原始输出/代码片段污染长期记忆）；
  - 候选更新出现 URL 行占比过高时拒写（`url_line_overflow`，防止链接清单倒灌到长期记忆）；
  - 候选更新出现重复行占比过高时拒写（`duplicate_line_overflow`，防止重复 bullet/句子污染长期记忆）；
  - 候选更新相对当前内容“过度收缩”时拒写；
  - `##` 标题保留率过低时拒写（结构突变保护）；
  - 候选更新为“长纯段落且缺少结构标记（`##`/`-`）”时拒写（防止无结构文本污染）；
  - 候选更新出现“日期行占比过高（近期流水痕迹明显）”时拒写（防止事件日志倒灌到长期记忆）；
...
- 进展（2026-02-27）：
  ...
  - drop preview 新增 `Top candidate files`（默认前 3，按候选条数降序），便于快速定位受影响文件。
  ...
  - `memory-observe` 支持 `--preview-days N` 自定义 half-life 预估窗口（默认 30 天）。
...
- 已实施：`nanobot memory-audit --apply-safe`
  - 行为：自动开启 `--apply`，并注入安全默认参数（`--apply-recent-days 7` + half-life 30d + `--apply-abort-on-high-risk`）；
  - 定位：降低手工拼参数成本，优先走小范围灰度清理。
...
- 已实施：`nanobot memory-audit --archive-apply [--archive-apply-max-files N]`
  - 行为：将超出保留窗口（`--archive-keep-days`）的 daily files 移动到 `memory/archive/`，并写入 `memory/observability/daily-archive-metrics.jsonl`；
  - 定位：生命周期最小落地（可审计归档），先收敛“长期堆积”的 daily 文件体量。
