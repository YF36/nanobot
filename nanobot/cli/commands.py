...
def memory_audit(
    ...
    sanitize_metrics_summary: bool = typer.Option(
        ...
        help="Show memory_update sanitize metrics summary",
    ),
    sanitize_metrics_out: str = typer.Option(
        "",
    ...
    if sanitize_metrics_summary or sanitize_metrics_out:
        ...
        if sanitize_metrics_out:
            out = Path(sanitize_metrics_out).expanduser()
            out.parent.mkdir(parents=True, exist_ok=True)
            out.write_text(sanitize_md, encoding="utf-8")
            console.print(f"[green]✓[/green] Wrote sanitize metrics summary: {out}")
...
def gateway(
    ...
    agent = AgentLoop(
        ...
        memory_daily_sections_mode=config.agents.defaults.memory_daily_sections_mode,
        brave_api_key=config.tools.web.search.api_key or None,
        web_search_max_results=config.tools.web.search.max_results,
        web_search_timeout_s=config.tools.web.search.timeout_s,
        web_search_max_retries=config.tools.web.search.max_retries,
        ...
        audit_tool_calls=config.agents.defaults.audit_tool_calls,
    ...
    if message:
        ...
        async def run_interactive():
            ...
            try:
                ...
                await agent_loop.wait_stopped(timeout=30.0)
